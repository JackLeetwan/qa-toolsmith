...
try {
  ...
  if (error) {
    ...
    if (
      ...
      console.error(
        "   → Use @mailinator.com, @guerrillamail.com, or real domain instead",
      );
    }
    if (error.message?.includes("Email not allowed")) {
      console.error("   → Supabase may be blocking this email domain");
      console.error(
        "   → Try using a different email domain or real email service",
      );
    }
    if (
      ...
      console.error(
        "   → Email confirmation is required in Supabase Auth settings",
      );
